...
final class CoreDataLocalStore: LocalStore {
    ...
    func upsertProfile(_ dto: ProfileDTO) {
        bg.perform {
            do {
                ...
                try self.bg.save()
                print("✅ CoreDataLocalStore: saved profile dto for uid=\(dto.userId)")
            } catch {
                print("❌ CoreDataLocalStore: save error \(error)")
            }
        }
    }
}
...
private extension CoreDataLocalStore {
