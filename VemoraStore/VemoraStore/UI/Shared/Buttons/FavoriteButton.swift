...
///
/// Отвечает за:
/// - отображение состояния избранного (в списке или карточке товара);
/// - анимированное переключение иконки при изменении состояния;
/// - адаптацию размеров и внешнего вида иконки под дизайн.
///
/// Структура и поведение:
/// - основан на `UIButton` с конфигурацией `.plain`;
/// - использует системные SF Symbols (`heart`, `heart.fill`);
/// - при смене состояния применяет анимацию `crossDissolve`;
/// - имеет фиксированный intrinsic size, зависящий от `pointSize`.
///
/// Публичный API:
/// - `setFavorite(_:animated:)` — переключает состояние избранного;
/// - `init(pointSize:)` — инициализация с кастомным размером символа.
///
/// Используется:
/// - на карточках товара (`ProductCardCell`);
/// - на экране деталей продукта (`ProductDetailsViewController`);
/// - в избранных товарах (`FavoritesViewController`).
