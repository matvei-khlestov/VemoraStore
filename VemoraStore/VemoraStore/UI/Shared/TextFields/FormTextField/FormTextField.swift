...
/// - `UITextField` — ввод;
/// - `UIButton` — «глаз» (для пароля);
/// - `UILabel` — строка ошибки;
/// - `UIStackView` — вертикальная компоновка.
///
/// Поведение:
/// - эмитит `onTextChanged` при изменении текста;
...
/// - `text` — геттер/сеттер значения;
/// - `setPhoneE164(_:)` — установить номер в E.164;
/// - `showError(_:force:)` — показать/скрыть ошибку.
///
/// Использование:
/// - экраны регистрации/входа;
/// - формы профиля и чекаута.
