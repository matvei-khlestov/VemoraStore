...
///
/// Структура UI:
/// - `UILabel` — обычный текст;
/// - `UnderlinedButton` — кликабельная ссылка;
/// - `UIStackView` — горизонтальное размещение элементов.
///
/// Публичный API:
/// - `onTap` — обработчик нажатия на ссылку;
/// - `configure(label:button:)` — обновление текстов;
/// - `button` — доступ к кнопке для внешней подписки.
///
/// Используется:
/// - в формах и экранах авторизации (например, “Уже есть аккаунт? Войти”);
/// - для интерактивных текстовых пар с ссылкой.
