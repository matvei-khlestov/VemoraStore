...
/// Кастомный компонент `BadgeView`.
///
/// Отвечает за:
/// - отображение короткой метки (бейджа) с текстом и цветом фона;
/// - автоматическое подстраивание размеров под содержимое.
///
/// Структура UI:
/// - `UILabel` по центру вью, внутри отступы;
/// - фон — закруглённый прямоугольник.
///
/// Публичный API:
/// - `configure(text:color:)` — задаёт текст и цвет бейджа.
///
/// Особенности:
/// - используется для статусов, категорий, скидок и т.п.;
/// - легко интегрируется в списки и карточки.
