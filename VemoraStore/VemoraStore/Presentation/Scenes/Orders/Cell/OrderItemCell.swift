...
/// Ячейка `OrderItemCell` для экранов истории и деталей заказа.
///
/// Основные задачи:
...
/// - Загружает изображение товара по URL (`UIImageView.loadImage`).
///
/// Особенности:
/// - Адаптивная вёрстка на UIStackView + Auto Layout;
/// - Отдельный тонкий разделитель с возможностью скрытия (`showsSeparator`);
/// - Корректная подготовка к переиспользованию (сброс текстов и изображения).
