...
/// Контроллер `SignInViewController` отвечает за экран авторизации пользователя.
///
/// Отвечает за:
/// - отображение формы входа с полями e-mail и пароля;
/// - привязку данных к `SignInViewModelProtocol` через Combine;
/// - управление состоянием кнопки «Войти»;
/// - переходы на экраны восстановления пароля и регистрации.
///
/// Архитектура и ответственность:
/// - Использует MVVM: UI управляется через Combine-паблишеры из ViewModel;
