...
/// Контроллер `ResetPasswordViewController` для экрана восстановления пароля.
///
/// Отвечает за:
/// - ввод e-mail и биндинг с `ResetPasswordViewModelProtocol`;
/// - отображение ошибок валидации и состояния кнопки отправки;
/// - отправку запроса на сброс пароля и показ результата (алерт);
/// - маршрутизацию через колбэки `onBack` и `onDone`;
/// - управление клавиатурой и скрытие её по тапу.
///
/// Особенности:
/// - обновление UI через Combine на главном потоке;
/// - безопасный сабмит из клавиатуры (`submitFromKeyboard`);
/// - верстка кодом без сторибордов.
