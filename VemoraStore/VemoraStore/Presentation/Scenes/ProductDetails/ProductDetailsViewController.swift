...
/// Контроллер `ProductDetailsViewController` для экрана карточки товара.
///
/// Отвечает за:
/// - отображение изображения, описания, цены и названия товара;
/// - взаимодействие с `ProductDetailsViewModelProtocol`
///   (биндинг данных, форматирование цены, обновление состояния);
/// - обработку действий пользователя: добавление/удаление из корзины,
///   добавление/удаление из избранного;
/// - показ актуального состояния кнопок «В корзине» и «Избранное»
///   с лёгкой анимацией и haptic-откликом;
/// - обновление интерфейса при изменениях в Combine-потоках ViewModel.
///
/// Контроллер не содержит бизнес-логики — она инкапсулирована во ViewModel.
/// Особенности:
/// - асинхронная загрузка изображения через Kingfisher;
/// - реактивное обновление UI через Combine;
/// - чистая верстка на UIStackView и Auto Layout;
/// - обработка всех действий пользователя с тактильной обратной связью.
