...
/// Контроллер `CatalogFilterViewController` для экрана фильтра каталога.
///
/// Отвечает за:
/// - отображение секций: категории, бренды, цена (`UICollectionView`);
/// - компоновку через Compositional Layout (капсулы и поле цен);
/// - биндинг с `CatalogFilterViewModelProtocol` (данные и счетчик найденных);
/// - синхронизацию выделений с текущим `FilterState`;
/// - ввод мин/макс цены и валидную передачу текста в VM;
/// - управление нижней панелью применения фильтров (`FilterBottomBar`);
/// - обработку клавиатуры и корректные отступы контента;
/// - действия пользователя: выбор/сброс фильтров, применение, возврат.
///
/// Колбэки:
/// - `onBack` — закрытие экрана;
/// - `onApply` — возврат выбранного `FilterState` наружу.
///
/// Особенности:
/// - многократный выбор в секциях капсул;
/// - аккуратная подсветка выбранных капсул без лишних `reload`;
/// - а11y-метки для капсул и полей цен.
/// Контроллер не содержит бизнес-логики — всё в ViewModel и репозиториях.
