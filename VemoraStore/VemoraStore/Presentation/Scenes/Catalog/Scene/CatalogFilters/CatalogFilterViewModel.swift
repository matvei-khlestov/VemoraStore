...
/// ViewModel `CatalogFilterViewModel` для экрана фильтрации каталога.
///
/// Основные задачи:
/// - Предоставляет списки категорий и брендов из `CatalogRepository`;
/// - Отслеживает выбор пользователя (категории, бренды, диапазон цен);
/// - Формирует и публикует актуальное состояние фильтров (`FilterState`);
/// - Подсчитывает количество найденных товаров при изменении фильтров.
///
/// Особенности:
/// - Реализует реактивную модель через Combine (`@Published`, `CurrentValueSubject`);
/// - Сортирует категории и бренды по алфавиту;
/// - Позволяет сбросить все фильтры одной командой (`reset`);
/// - При пустых фильтрах останавливает наблюдение за товарами для оптимизации.
///
/// Реактивность:
/// - `statePublisher` — передаёт текущее состояние фильтров;
/// - `foundCountPublisher` — обновляет количество найденных товаров;
/// - `categories` и `brands` — реактивно обновляются при изменениях в репозитории.
