...
/// Контроллер `AboutViewController` для экрана «О нас».
////
//// Отвечает за:
/// - показ вводного текста и списка буллетов;
/// - компоновку через `UIScrollView` + `UIStackView`;
/// - настройку навбара с кнопкой «Назад»;
/// - генерацию строк с иконкой и двумя лейблами.
////
//// Особенности:
/// - поддержка Dynamic Type для всех лейблов;
/// - адаптивные отступы и автолейаут;
/// - переинициализация списка буллетов без `reload`.
////
//// Колбэки:
