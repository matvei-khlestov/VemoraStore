...
/// Контроллер `PrivacyPolicyViewController` для экрана "Политика конфиденциальности".
///
/// Отвечает за:
/// - отображение текста политики конфиденциальности в виде прокручиваемого `UITextView`;
...
/// - текст форматируется с помощью `NSMutableParagraphStyle`;
/// - контент подставляется из `PrivacyPolicyText.body`.
