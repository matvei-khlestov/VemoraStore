...
/// Координатор `CatalogCoordinator` управляет основным сценарием работы с каталогом товаров.
///
/// Отвечает за:
/// - инициализацию и показ `CatalogViewController`;
/// - навигацию к экранам деталей товара, категории или фильтрации;
/// - передачу данных между экранами и синхронизацию состояния фильтров.
///
/// Особенности:
/// - использует фабрики `ViewModelBuildingProtocol` и `CoordinatorBuildingProtocol` для создания зависимостей;
/// - применяет архитектуру `Coordinator` для изоляции навигационной логики от слоя представления;
/// - поддерживает реактивное обновление каталога при изменении фильтров (`FilterState`);
/// - обеспечивает масштабируемую навигацию с возможностью вложенных координаторов.
