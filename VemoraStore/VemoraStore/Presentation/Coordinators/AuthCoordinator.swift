...
/// Координатор `AuthCoordinator` управляет сценарием аутентификации пользователя.
///
/// Отвечает за:
/// - отображение контейнера авторизации (`AuthContainerViewController`), включающего экраны входа и регистрации;
/// - обработку навигации к экранам политики конфиденциальности и восстановления пароля;
/// - отслеживание состояния авторизации через `AuthServiceProtocol` и завершение сценария при успешном входе.
///
/// Особенности:
/// - использует `Combine` для реактивного мониторинга состояния пользователя (`isAuthorizedPublisher`);
/// - координирует вложенные потоки: `ResetPasswordCoordinator` и `PrivacyPolicyCoordinator`;
/// - предотвращает дублирование экранов с помощью флага `isShowingAuthContainer`;
/// - реализует обратный вызов `onFinish`, который вызывается при успешной авторизации пользователя.
