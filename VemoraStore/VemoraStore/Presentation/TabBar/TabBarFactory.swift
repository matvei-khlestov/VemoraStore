...
/// Фабрика `TabBarFactory` создаёт и настраивает навигационные контроллеры
/// и главный `UITabBarController` приложения.
///
/// Отвечает за:
/// - инициализацию вкладок с системными иконками и заголовками;
/// - установку корневых контроллеров для каждой вкладки;
/// - настройку внешнего вида `UITabBar`
///   (цвета текста и иконок для активного и неактивного состояния).
///
/// Особенности:
/// - обеспечивает единый стиль таб-бара по всему приложению;
/// - использует кастомные цвета (`.brightPurple`) из ассетов;
/// - поддерживает адаптацию под iOS 15+ через `scrollEdgeAppearance`.
